function Dot() {
  ...
  this.amplitude = 0.05; //the max distance to go up/down - this is proportional to width also
...
Dot.prototype.draw = function (environment, timeline) {
  ...
  if (this.greyPingExit) {
    ...
    if (this.pos.x > pingStart) {
      ...
      drawCircle(
        ...
        this.pos.x,
        this.pos.y,
        this.width * (distanceSoFar * distanceUnit) * 25,
        RGBA_darkGrey.strRGBA(1 - (distanceSoFar * distanceUnit) / 0.38)
      );
    }
  }
  ...
  if (this.greyPingVanish) {
    ...
    drawCircle(
      ...
      this.pos.x,
      this.pos.y,
      this.width * getViewportDPI() * timeline.getCurrentElapsed() * 9,
      RGBA_darkGrey.strRGBA(1 - timeline.getCurrentElapsed() * opacityUnit)
    );
  }
  ...
  if (this.text) {
    ...
    environment.context.fillText(
      this.text,
      this.pos.x + this.textOffsetX,
      this.pos.y + this.textOffsetY
    );
  }
};
